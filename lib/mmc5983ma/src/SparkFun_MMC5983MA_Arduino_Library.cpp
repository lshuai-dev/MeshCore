...
bool SFE_MMC5983MA::begin()
{
    bool success = mmc_io.begin();
    if (!success)
    {
...
int SFE_MMC5983MA::getTemperature()
{
    ...
    do
    {
        ...
        delay(1);
        timeOut--;
    } while ((!mmc_io.isRegisterBitSet(STATUS_REG, MEAS_T_DONE)) && (timeOut > 0));
...
uint32_t SFE_MMC5983MA::getMeasurementX()
{
    ...
    do
    {
        ...
        delay(1);
        timeOut--;
    } while ((!mmc_io.isRegisterBitSet(STATUS_REG, MEAS_M_DONE)) && (timeOut > 0));
