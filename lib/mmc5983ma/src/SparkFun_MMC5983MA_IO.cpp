...
static bool i2cProbe(TwoWire& wire, uint8_t addr) {
  wire.beginTransmission(addr);
  uint8_t error = wire.endTransmission();
  return (error == 0);
}

static TwoWire* getI2CBus()
{
  static TwoWire * gI2CBus = nullptr;
  static bool gI2CInited = false;
  if(!gI2CInited) {
    if(i2cProbe(Wire,I2C_ADDR)) {
      gI2CBus = &Wire;
    }
#if WIRE_INTERFACES_COUNT > 1
    else if(i2cProbe(Wire1,I2C_ADDR)) {
      gI2CBus = &Wire1;
    }
#endif
    gI2CInited = true;
...
bool SFE_MMC5983MA_IO::isRegisterBitSet(const uint8_t registerAddress, const uint8_t bitMask)
{
    uint8_t value = 0;
