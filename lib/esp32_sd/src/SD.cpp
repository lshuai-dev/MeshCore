...
bool SDFS::begin(uint8_t ssPin, SPIClass &spi, uint32_t frequency, const char * mountpoint, uint8_t max_files, bool format_if_empty)
{
...
#if !defined(HELTEC_SD_BUS_PRECONFIGURED) || !HELTEC_SD_BUS_PRECONFIGURED
    spi.begin();
#endif
    ...
    if(_pdrv == 0xFF) {
        ...
        Serial.printf("[spi] SD.begin sdcard_init failed CS=%u\n", (unsigned)ssPin);
#endif
        return false;
    }
...
#if defined(MESH_DEBUG) && MESH_DEBUG
        Serial.printf("[spi] SD.begin sdcard_mount failed CS=%u card_type=%u (13=FAT missing)\n",
                      (unsigned)ssPin, (unsigned)sdcard_type(_pdrv));
#endif
